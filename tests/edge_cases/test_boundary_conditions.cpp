...
int main() {
    ...
    RUN_TEST("large_numbers", [](test_utils::TestContext& ctx) {
        ...
        ctx.expect_equal(calculator::add(large, large), 2e10);
        ctx.expect_equal(calculator::sub(large, large), 0.0);
        ctx.expect_equal(calculator::mul(large, 2.0), 2e10);
        ctx.expect_optional_equal(calculator::divi(large, 2.0), 5e9);
    });

    // Тесты для очень маленьких чисел
    RUN_TEST("small_numbers", [](test_utils::TestContext& ctx) {
        const double small = 1e-10;
        ctx.expect_equal(calculator::add(small, small), 2e-10);
        ctx.expect_equal(calculator::sub(small, small), 0.0);
        ctx.expect_equal(calculator::mul(small, 2.0), 2e-10);
        ctx.expect_optional_equal(calculator::divi(small, 2.0), 5e-11);
    });

    // Тесты для чисел с максимальной точностью
    RUN_TEST("precision_limits", [](test_utils::TestContext& ctx) {
        const double eps = std::numeric_limits<double>::epsilon();
        ctx.expect_equal(calculator::add(1.0, eps), 1.0 + eps);
        ctx.expect_equal(calculator::sub(1.0, eps), 1.0 - eps);
        ctx.expect_equal(calculator::mul(1.0, eps), eps);
    });
    ...
    RUN_TEST("infinity_nan", [](test_utils::TestContext& ctx) {
        ...
        ctx.check(std::isinf(calculator::add(inf, 1.0)), "add with infinity should return infinity");
        ctx.check(std::isinf(calculator::sub(inf, 1.0)), "sub with infinity should return infinity");
        ctx.check(std::isinf(calculator::mul(inf, 2.0)), "mul with infinity should return infinity");
        
        // Проверяем, что операции с NaN возвращают NaN
        ctx.check(std::isnan(calculator::add(nan, 1.0)), "add with NaN should return NaN");
        ctx.check(std::isnan(calculator::sub(nan, 1.0)), "sub with NaN should return NaN");
        ctx.check(std::isnan(calculator::mul(nan, 2.0)), "mul with NaN should return NaN");
    });
    ...
    RUN_TEST("division_edge_cases", [](test_utils::TestContext& ctx) {
        ...
        auto result1 = calculator::divi(tiny, huge);
        ...
        auto result2 = calculator::divi(huge, tiny);
        ...
        ctx.expect_nullopt(calculator::divi(1.0, 0.0));
    ...
    RUN_TEST("power_edge_cases", [](test_utils::TestContext& ctx) {
        ...
        ctx.expect_optional_equal(calculator::powd(1.0, huge), 1.0);
        ctx.expect_optional_equal(calculator::powd(huge, 0.0), 1.0);
        ctx.expect_optional_equal(calculator::powd(tiny, 1.0), tiny);
    ...
    RUN_TEST("logarithm_edge_cases", [](test_utils::TestContext& ctx) {
        ...
        ctx.expect_optional_equal(calculator::logb(huge, huge), 1.0);
        ctx.expect_optional_equal(calculator::logb(tiny, tiny), 1.0);
        ctx.expect_nullopt(calculator::logb(0.0, 2.0));    // Логарифм нуля
        ctx.expect_nullopt(calculator::logb(1.0, 1.0));    // Логарифм по основанию 1
        ctx.expect_nullopt(calculator::logb(-1.0, 2.0));   // Логарифм отрицательного числа
    });
    ...
    RUN_TEST("modulo_edge_cases", [](test_utils::TestContext& ctx) {
        ...
        ctx.expect_optional_equal(calculator::mod(huge, 1.0), 0.0);
        ctx.expect_optional_equal(calculator::mod(tiny, 1.0), tiny);
        ctx.expect_nullopt(calculator::mod(1.0, 0.0));
    ...
    RUN_TEST("performance_large_calculations", [](test_utils::TestContext& ctx) {
        ...
        auto result = calculator::powd(base, exp);
